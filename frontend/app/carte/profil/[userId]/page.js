...
function Inner() {
  ...
  useEffect(() => {
    api.get(`/public/profil/${userId}`, { params: { token } })
        ...
        {data && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="card text-center">
              ...
              <p className="text-sm text-wave-500">Membre · {data.groupes.length} groupe{data.groupes.length > 1 ? "s" : ""}</p>
            </div>

            <div className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-wave-500 px-1">Associations</h3>
              {data.groupes.map((g, i) => (
                <motion.div key={g.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                  <Link
                    href={`/carte/profil/${userId}/groupe/${g.id}?token=${token}`}
