    ...
    >
      {welcome && (
        <motion.div
          ...
          className="mb-4 rounded-2xl bg-brand-50 p-4 text-sm text-brand-600"
        >
      ...
      {loading ? (
        <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-28 w-[280px] shrink-0 animate-pulse rounded-3xl bg-wave-100/60"
            />
          ))}
        </div>
      ) : (
        <>
          ...
          <div className="-mx-4 px-4 sm:mx-0 sm:px-0">
            <div className="flex gap-4 overflow-x-auto pb-4 pt-1 snap-x snap-mandatory hide-scrollbar sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:overflow-visible">
              <div className="w-[80vw] max-w-[280px] shrink-0 snap-center sm:w-auto">
                <KPI
                  Icon={Wallet}
                  label="Solde de la caisse"
                  value={formatMoney(dash.solde_caisse, groupe?.devise)}
                  tone="dark"
                />
              </div>
              <div className="w-[80vw] max-w-[280px] shrink-0 snap-center sm:w-auto">
                <KPI
                  ...
                  value={`${dash.taux_collecte}%`}
                  progress={dash.taux_collecte}
                />
              </div>
              <div className="w-[80vw] max-w-[280px] shrink-0 snap-center sm:w-auto">
                <KPI
                  ...
                  value={`${dash.a_jour}/${dash.nb_membres_actifs}`}
                />
              </div>
              <div className="w-[80vw] max-w-[280px] shrink-0 snap-center sm:w-auto">
                <KPI
          ...
          {dash.nb_demandes > 0 && dash.demandes_en_attente && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mt-2 rounded-2xl bg-amber-50 p-4 border border-amber-200"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm font-bold text-amber-800">
                    {dash.nb_demandes} demande{dash.nb_demandes > 1 ? "s" : ""}{" "}
                    de paiement en attente
                  </p>
                  <p className="text-xs text-amber-600">
                    Vérifiez les reçus soumis par vos membres.
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                {dash.demandes_en_attente.slice(0, 3).map((d) => (
                  <div
                    key={d.id}
                    className="flex items-center justify-between rounded-xl bg-white p-3 shadow-sm ring-1 ring-amber-100"
                  >
                    <div>
                    ...
                    <button
                      onClick={() => setDemandesOpen(true)}
                      className="rounded-xl bg-amber-100 px-3 py-1.5 text-xs font-bold text-amber-700 transition hover:bg-amber-200"
                    >
                      Examiner
                    </button>
                  </div>
                ...
                {dash.nb_demandes > 3 && (
                  <button
                    onClick={() => setDemandesOpen(true)}
                    className="w-full rounded-xl bg-amber-100/50 py-2 text-xs font-bold text-amber-700 transition hover:bg-amber-100"
                  >
          ...
          {maCotisation && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mt-4 rounded-2xl bg-white p-4 shadow-soft ring-1 ring-wave-100"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-wave-50 text-wave-700">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-wave-400">
                      {fcfa(maCotisation.montant_verse)} versés sur{" "}
                      {fcfa(maCotisation.montant_du)}
                    </p>
                  </div>
                </div>
                {maCotisation.reste_a_payer > 0 && (
                  <button
                    onClick={() => setManagerPayOpen(true)}
                    className="rounded-xl bg-brand-50 px-4 py-2.5 text-xs font-bold text-brand-600 transition hover:bg-brand-100 whitespace-nowrap"
                  >
          ...
          <div className="-mx-4 px-4 sm:mx-0 sm:px-0 mt-4">
            <h3 className="mb-3 font-display text-base font-bold text-wave-900">
              Actions rapides
            </h3>
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory hide-scrollbar sm:grid sm:grid-cols-5 sm:overflow-visible"
            >
              <button
                onClick={() => setPayOpen(true)}
                className="card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5"
              >
                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-50 text-brand-600">
                  <Plus className="h-6 w-6" />
                </span>
                <div>
                  <p className="text-sm font-bold">Encaisser</p>
                  <p className="text-xs text-wave-500">Cash/Mobile Money</p>
                </div>
              </button>
              <button
                ...
                className="card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5"
              >
                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-wave-50 text-wave-700">
                  <UserPlus className="h-6 w-6" />
                </span>
                <div>
                  <p className="text-sm font-bold">Inviter</p>
                  <p className="text-xs text-wave-500">Lien ou SMS</p>
                </div>
              </button>
              <button
                onClick={() => router.push(`/app/g/${id}/membres?action=add`)}
                className="card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5"
              >
                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-50 text-brand-600">
                ...
                <div>
                  <p className="text-sm font-bold">Ajouter</p>
                  <p className="text-xs text-wave-500">Un membre</p>
                </div>
              </button>
              <button
              ...
              >
                ...
                <div>
                  <p className="text-sm font-bold">Tarifs</p>
                  <p className="text-xs text-wave-500">Cotisation</p>
                </div>
              </button>
              <button
                onClick={() =>
                  downloadWithToken(
                    `/groupes/${id}/export/pdf`,
                    `rapport-${id}.pdf`,
                  )
                }
                className="card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-wave-500">Bilan PDF</p>
                </div>
              </button>
              <button
                onClick={() =>
                  downloadWithToken(
                    `/groupes/${id}/export/csv`,
                    `paiements-${id}.csv`,
                  )
                }
                className="card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-wave-500">CSV complet</p>
                </div>
              </button>
              {!dash.has_payments && (
                <button
                  onClick={() => setDeleteOpen(true)}
                  className="card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5 border border-red-100 bg-red-50/30"
                >
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-red-100 text-red-600">
                    <Trash2 className="h-6 w-6" />
                  </span>
                  <div>
                    <p className="text-sm font-bold text-red-700">Supprimer</p>
                    <p className="text-xs text-red-500">Ce groupe</p>
                  </div>
                </button>
              )}
              <button
                onClick={() => setDemandesOpen(true)}
                className={`card shrink-0 w-[140px] snap-center sm:w-auto flex flex-col items-center gap-2 py-4 text-center transition hover:-translate-y-0.5 ${dash.nb_demandes > 0 ? "ring-2 ring-amber-400" : ""}`}
              >
                <span
                  className={`grid h-12 w-12 place-items-center rounded-2xl ${dash.nb_demandes > 0 ? "bg-amber-50 text-amber-600" : "bg-wave-50 text-wave-700"}`}
                >
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-3">
            <div className="card lg:col-span-2 !p-0 overflow-hidden shadow-xl shadow-slate-200/50 border-slate-100">
              <div className="flex items-center justify-between p-6 border-b border-slate-50 bg-slate-50/30">
                ...
                <Link
                  href={`/app/g/${id}/caisse`}
                  className="rounded-full bg-white px-4 py-2 text-xs font-black text-wave-700 shadow-sm ring-1 ring-slate-100 hover:bg-wave-50 transition-colors"
                >
                  Voir tout
                </Link>
              </div>
              <div className="p-6">
                {dash.dernieres_transactions.length === 0 ? (
                  <div className="py-12 text-center">
                    ...
                    <button
                      onClick={() => setPayOpen(true)}
                      className="mt-4 text-xs font-black text-wave-600 uppercase tracking-wider"
                    >
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                            {t.date_paiement} • {t.mode}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-base font-black text-emerald-600">
                            +{formatMoney(t.montant, groupe?.devise)}
                          </p>
            ...
            <div className="card !p-0 overflow-hidden shadow-xl shadow-slate-200/50 border-slate-100">
              <div className="p-6 wave-bg text-white">
                ...
                <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">
                  {dash.periode
                    ? `Échéance ${dash.periode.echeance}`
              ...
              <div className="p-6">
                <div className="relative">
                  <div className="flex items-end justify-between mb-2">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-sm font-black text-emerald-600">
                        {formatMoney(dash.total_recu, groupe?.devise)}
                      </span>
                    </div>
                  </div>
                  <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${dash.taux_collecte}%` }}
                  ...
                  <p className="mt-2 text-right text-[10px] font-bold text-slate-400">
                    Objectif : {formatMoney(dash.total_attendu, groupe?.devise)}
                  </p>
                </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <Link
              href={`/app/g/${id}/membres`}
              className="card flex items-center justify-between hover:-translate-y-0.5"
            >
              <div className="flex items-center gap-3">
                <Users className="h-5 w-5 text-wave-700" />
                <span className="font-semibold">Gérer les membres</span>
              </div>
              <span className="rounded-full bg-wave-50 px-2 py-0.5 text-xs font-semibold text-wave-700">
                {dash.nb_membres}
              </span>
            </Link>
            <Link
            ...
            >
              <div className="flex items-center gap-3">
                <Wallet className="h-5 w-5 text-wave-700" />
                <span className="font-semibold">Caisse & grand livre</span>
              </div>
              <span className="rounded-full bg-brand-50 px-2 py-0.5 text-xs font-semibold text-brand-600">
                {formatMoney(dash.solde_caisse, groupe?.devise)}
              </span>
            </Link>
            <button
              onClick={() => setInviteOpen(true)}
            ...
            >
              <div className="flex items-center gap-3">
                <Send className="h-5 w-5 text-wave-700" />
                <span className="font-semibold">Invitations en attente</span>
              </div>
              <span className="rounded-full bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700">
                {dash.invitations_en_attente}
              </span>
            </button>
          </div>
        </>
      ...
      {managerPayOpen && (
        <ManagerPayModal
          groupeId={id}
          membreId={maCotisation?.membre?.id}
          onClose={() => {
            setManagerPayOpen(false);
            loadAll();
          }}
        />
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <select
            ...
            value={f.devise}
            onChange={(e) => setF({ ...f, devise: e.target.value })}
          >
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setF({
                ...f,
                montant_standard: e.target.value
          ...
          <p className="text-xs text-wave-500 mt-1">
            Ce montant s'appliquera par défaut à chaque période.
          </p>
        </div>
        <div className="pt-2 border-t border-wave-100">
          <label className="flex items-center gap-2 cursor-pointer mb-2">
            <input
              type="checkbox"
              checked={f.adhesion_active}
              onChange={(e) =>
                setF({ ...f, adhesion_active: e.target.checked })
              }
              className="h-5 w-5 rounded text-wave-600 focus:ring-wave-600"
            />
            <span className="text-base font-bold text-wave-800">
              Frais d'adhésion activés
            </span>
          </label>
          {f.adhesion_active && (
            <motion.div
            ...
            >
              <label className="label mt-2">
                Montant de l'adhésion ({f.devise})
              </label>
              <input
                type="number"
                min="0"
                ...
                onChange={(e) =>
                  setF({
                    ...f,
                    adhesion_montant: e.target.value
                      ? parseInt(e.target.value)
                      : "",
                  })
                }
...
function KPI({ Icon, label, value, tone, progress }) {
  const toneCls =
    tone === "dark"
      ? "wave-bg text-white ring-0"
      : tone === "danger"
        ? "bg-red-50 text-red-700 ring-red-100"
        : "bg-white text-wave-900";
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className={`rounded-3xl p-4 shadow-soft ring-1 ring-wave-100 ${toneCls}`}
    >
      <div className="flex items-center gap-3">
        <span
          className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${tone === "dark" ? "bg-white/15" : "bg-wave-50 text-wave-700"}`}
        >
      ...
      {progress != null && (
        <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-white/20">
          <motion.div
            ...
            className={`h-full rounded-full ${tone === "dark" ? "bg-white" : "wave-bg"}`}
          />
        </div>
      )}
    </motion.div>
...
function PayModal({ groupeId, onClose }) {
  ...
  useEffect(() => {
    api
      ...
      .then((r) => setMembres(r.data.membres));
  }, [groupeId]);
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setF({ ...f, date_paiement: e.target.value })}
              />
            </div>
          </div>
          <div>
...
function InviteModal({ groupeId, onClose }) {
  ...
  useEffect(() => {
    api
      ...
      .then((r) => setGlobalLink(r.data.link));
  }, [groupeId]);
    ...
    <Modal title="Inviter un membre à rejoindre le groupe" onClose={onClose}>
      <div className="mb-4 grid grid-cols-2 rounded-2xl bg-wave-50 p-1 text-xs font-semibold">
        <button
          ...
          className={`rounded-xl px-3 py-2 transition ${mode === "global" ? "bg-white text-wave-900 shadow-sm" : "text-wave-500"}`}
        >
          Lien public
        </button>
        <button
          onClick={() => setMode("individuel")}
          className={`rounded-xl px-3 py-2 transition ${mode === "individuel" ? "bg-white text-wave-900 shadow-sm" : "text-wave-500"}`}
        >
          Individuel
        </button>
      </div>
      ...
      {mode === "global" ? (
        <div className="space-y-3">
          <div className="rounded-3xl bg-wave-50 p-4 text-sm text-wave-600">
            Toute personne avec ce lien peut rejoindre le groupe en remplissant
            ses informations.
          </div>
          {globalLink ? (
            <>
              <div className="flex items-center gap-2 rounded-2xl bg-wave-50 p-3">
                ...
                <input
                  ...
                  value={globalLink.url}
                />
                <button
                  onClick={() => copyLink(globalLink.url)}
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-wave-500">
                <div className="rounded-2xl bg-wave-50 p-3">
                  Utilisé :{" "}
                  <span className="font-bold text-wave-800">
                    {globalLink.uses_count}
                  </span>
                </div>
                <div className="rounded-2xl bg-wave-50 p-3">
                  Expire :{" "}
                  <span className="font-bold text-wave-800">
                    {globalLink.expires_at
                      ? new Date(globalLink.expires_at).toLocaleDateString(
                          "fr-FR",
                        )
                      : "Jamais"}
                  </span>
              ...
              <a
                href={`https://wa.me/?text=${encodeURIComponent(globalLink.url)}`}
                target="_blank"
            ...
            <button
              ...
              className="btn-primary w-full !py-3"
            >
              {loading ? "Génération..." : "Générer le lien public"}
            </button>
          )}
          {err && (
        ...
        <div className="space-y-3 text-center">
          ...
          <p className="text-xs text-wave-600">
...
function DemandesModal({ groupeId, onClose }) {
  ...
  async function openPreuve(paiementId) {
    ...
    try {
      const response = await api.get(
        `/groupes/${groupeId}/paiements/${paiementId}/preuve`,
        {
          responseType: "blob",
        },
      );
  ...
  async function doRefuser(id) {
    setActionLoading(id);
    try {
      await api.post(`/groupes/${groupeId}/paiements/${id}/refuser`, {
          ...
          {loading ? (
            <div className="space-y-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="h-20 animate-pulse rounded-2xl bg-wave-100/60"
                />
              ))}
            </div>
          ) : demandes.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-12">
              <span className="grid h-14 w-14 place-items-center rounded-full bg-wave-50 text-wave-400">
                <CheckCircle2 className="h-7 w-7" />
              </span>
              <p className="text-sm font-semibold text-wave-500">
            ...
            demandes.map((d) => (
              <div
                key={d.id}
                className="rounded-2xl bg-wave-50 p-4 border border-wave-100"
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-bold text-wave-900">
                      {d.membre?.prenom} {d.membre?.nom}
                    </p>
                    <p className="text-xs text-wave-500">
                      {fmtDateGestionnaire(d.date_paiement)} • {fcfa(d.montant)}
                    </p>
                    <p className="text-xs text-wave-400 mt-0.5">
                      Il y a {timeAgo(d.created_at)}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {d.mode && OPERATEURS_MAP[d.mode] && (
                        <span
                          className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold ${OPERATEURS_MAP[d.mode].cls}`}
                        >
                          {OPERATEURS_MAP[d.mode].nom}
                        </span>
                      )}
                      <span
                        className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold ${d.type === "adhesion" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                      >
                        {d.type === "adhesion"
                          ? "Frais d'adhésion"
                          : d.type === "cotisation"
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() =>
                        setRejectId(rejectId === d.id ? null : d.id)
                      }
                      className="rounded-xl bg-red-500 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-red-600"
                    >
                      Refuser
                    </button>
                  </div>
                ...
                {d.preuve_path && (
                  <button
                    onClick={() => openPreuve(d.id)}
                    disabled={imageLoading}
                    className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-semibold text-wave-700 transition hover:bg-brand-50 border border-wave-200 w-full disabled:opacity-50"
                  >
                    <Eye className="h-4 w-4" />{" "}
                    {imageLoading ? "Chargement..." : "Voir la preuve"}
                  </button>
                )}
                {rejectId === d.id && (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => doRefuser(d.id)}
                        disabled={actionLoading === d.id}
                        className="flex-1 rounded-xl bg-red-500 px-3 py-2 text-xs font-bold text-white transition hover:bg-red-600 disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="rounded-xl bg-wave-100 px-3 py-2 text-xs font-semibold text-wave-600"
                      >
                        Annuler
                      </button>
                    </div>
      ...
      {imagePreview && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={closePreview}
              className="absolute -top-3 -right-3 grid h-10 w-10 place-items-center rounded-full bg-white shadow-lg text-wave-700"
            >
              <X className="h-5 w-5" />
            </button>
          </motion.div>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-red-50 p-4 text-sm text-red-700 font-semibold border border-red-100">
          <p>
            ...
            <strong>{groupeName}</strong> ?
          </p>
          <p className="mt-2 text-xs font-normal">
            Cette action est définitive et irréversible. Elle ne peut être
            effectuée que si aucun paiement n'a été enregistré.
          </p>
        </div>
        {err && (
          <motion.p
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="btn-primary flex-1 !py-3 !bg-red-500 hover:!bg-red-600 text-white border-0"
          >
