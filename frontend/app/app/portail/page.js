...
import { api, auth, fcfa, API_BASE } from "../../lib/api";
import AppShell from "../../components/AppShell";

export default function PortailGestionnaire() {
  ...
  useEffect(() => {
    ...
    if (u.role !== "gestionnaire") { router.push("/app"); return; }

    api.get("/carte/portail").then((r) => {
      setData(r.data);
  ...
  function downloadPdf() {
    ...
    fetch(`${API_BASE}/carte/portail/pdf`, {
      headers: { Authorization: `Bearer ${token}` }
    }).then(r => r.blob()).then(b => {
      const u = URL.createObjectURL(b);
      const a = document.createElement("a");
      a.href = u; a.download = "carte-gestionnaire.pdf"; a.click();
      URL.revokeObjectURL(u);
    });
  }

  if (loading) return (
    <AppShell title="Ma carte" role="gestionnaire" back>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center">
        ...
        <div className="relative w-full max-w-[340px] overflow-hidden rounded-3xl bg-[#1e40af] p-6 text-white shadow-2xl">
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            <div>
              <p className="text-[10px] uppercase tracking-[0.2em] text-wave-300">Carte Gestionnaire</p>
              <h3 className="font-display text-lg font-extrabold leading-tight text-white">Portail Groupes</h3>
            </div>
          ...
          <div className="relative z-10 flex items-end justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-wave-300 uppercase tracking-wider mb-1">Gestionnaire</p>
          ...
          <div className="relative z-10 mt-6 border-t border-white/10 pt-3 flex justify-between items-center">
            <p className="text-[9px] text-wave-400">Scanner pour voir tous les groupes</p>
        ...
        <p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-wave-400">
          <ShieldCheck className="h-3.5 w-3.5" /> Scannez le QR pour voir tous vos groupes
        </p>

        {/* Groupes liés */}
        <div className="mt-8 w-full max-w-[340px]">
          <h3 className="text-sm font-bold text-wave-900 mb-3">Mes groupes ({data?.groupes?.length || 0})</h3>
          <div className="space-y-2">
            {data?.groupes?.map(g => (
              <Link key={g.id} href={`/app/g/${g.id}`} className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-soft ring-1 ring-wave-100 transition hover:-translate-y-0.5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-wave-500">{g.membres_count} membre{g.membres_count > 1 ? "s" : ""} · {g.devise}</p>
                  </div>
                </div>
